...
exports.followUpServiceData = async (req, res, next) => {
  try {
    ...
    // Update the service dynamically with new service code
    updates.serviceCode = serviceCode;
    
    // Create a message to send to the user
    const message = "Your service inquiry code is: " + serviceCode;
    updates.message = message;

    await service.update(updates);

    // Send email with service inquiry code
    const emailContent = sendServiceCodeTemplate(service.fullName, serviceCode, message);
...
// @route   GET /api/v1/services/inquire/:code
exports.inquireServiceByCode = async (req, res, next) => {
  try {
    const { code } = req.params;

    // Find service by serviceCode (not id)
    const service = await Service.findOne({ where: { serviceCode: code } });
